...
export default function FFmpegDemo() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (errorMsg.includes('SharedArrayBuffer') || errorMsg.includes('多线程')) {
  ...
  const convertVideo = async (file: File) => {
    ...
    try {
      addLog(`开始转换: ${file.name} (${(file.size / 1024).toFixed(2)}KB)`, 'info');
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (file.size > maxSize) {
      addLog(`文件太大 (${(file.size / 1024 / 1024).toFixed(2)}MB)，建议小于 500MB`, 'warning');
      return;
    }
