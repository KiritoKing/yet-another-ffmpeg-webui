...
export function ModeSelect({ useMultiThread, onModeChange, disabled = false }: ModeSelectProps) {
  const isSharedArrayBufferAvailable = typeof SharedArrayBuffer !== 'undefined';

  return (
    <div className="flex items-center gap-2">
      <Label htmlFor="mode-select" className="text-sm font-medium whitespace-nowrap">
        运行模式:
      </Label>
      <TooltipProvider>
              ...
              <div className="space-y-2 text-sm">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    更快的处理速度，适合大文件和复杂操作。需要浏览器支持 SharedArrayBuffer（需要特殊的 HTTP 头配置）。
                  </p>
                </div>
                <div>
