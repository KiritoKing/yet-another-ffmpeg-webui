    ...
    <Collapsible open={isFilterOpen} onOpenChange={setIsFilterOpen}>
      ...
      <CollapsibleTrigger asChild>
        <div className="flex items-center justify-between cursor-pointer group py-2">
          ...
          >
            {selectedCategories.size === allCategories.length ? '保留一项' : '全选'}
          </Button>
        </div>
      </CollapsibleTrigger>
      ...
      <CollapsibleContent>
        <div className="flex flex-wrap gap-1.5 pb-2 pt-1">
          {allCategories.map((category) => {
            ...
            const count = presets.filter(p => (p.category || '未分类') === category).length;
            ...
            return (
              <Button
                ...
                title={isLastSelected ? '至少保留一个分类' : `点击${isSelected ? '取消' : ''}筛选 ${category}`}
              >
                {category}
                <Badge 
